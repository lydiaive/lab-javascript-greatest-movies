...
function getAllDirectors(moviesArray) {
    let directorArray = moviesArray.map(obj => obj.director);
    return directorArray
}

// Iteration 2: Steven Spielberg. The best? - How many drama movies did STEVEN SPIELBERG direct?
function howManyMovies(moviesArray) {
    let result = moviesArray.filter(obj => (obj.director === "Steven Spielberg" && obj.genre.includes("Drama")));
...
function scoresAverage(moviesArray) {
    ...
    let scoreArray = moviesArray.map(obj => obj.score);
    let scoreSum = scoreArray.reduce(function(a, c) {
        if (c) {
            return a + c;
        }   return a
    },0)
    return Math.round((scoreSum / scoreArray.length) * 100) / 100
}
// Iteration 4: Drama movies - Get the average of Drama Movies
 function dramaMoviesScore(moviesArray) {
    ...
    let dramaArray = moviesArray.filter(obj => (obj.genre.includes("Drama")));
    if (!dramaArray[0]) {
        return 0;
    } 
    let scoreDramaArray = dramaArray.map(obj => obj.score);
    let scoreSum = scoreDramaArray.reduce(function(a, c) {
        return a + c; });
    return Math.round((scoreSum / scoreDramaArray.length) * 100) / 100
    }
...
function orderAlphabetically(moviesArray) {
    let movieTitles = moviesArray.map(obj => obj.title);
    movieTitles.sort();
    if (movieTitles.length > 20) {
        result = movieTitles.splice(20)
    }   result = movieTitles
    return result
}
